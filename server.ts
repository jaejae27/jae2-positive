  ...
  app.post("/api/generate", async (req, res) => {
    try {
      ...
      if (!apiKey) {
        ...
        return res.status(500).json({ error: "서버 설정 오류가 발생했습니다. API 키를 확인해주세요." });
      }
...
4.  **핵심 강점 요약 (strength_summary)**: 모든 분석을 종합하여, 학생의 가장 빛나는 핵심 강점을 감동적이고 힘이 되는 한 문장으로 요약해주세요. 반드시 3인칭(예: "${name}님은...")으로 작성하며, 문장 끝에 "소중한"과 같은 수식어를 남발하지 말고 깔끔하게 "~하는 사람입니다" 또는 "~를 가졌어요"와 같이 마무리하세요.
5.  **어조**: 학생에게 직접 말을 거는 것처럼 친근하고 격려하는 말투를 사용해주세요. "나는 ~입니다"와 같은 1인칭 표현은 절대 금지하며, 반드시 "${name}님은 ... ~를 가졌어요!", "...할 수 있을 거예요!"와 같이 다정하면서도 예의를 갖춘 3인칭 말투를 사용하세요.
